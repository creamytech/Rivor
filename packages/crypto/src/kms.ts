...
export function createKmsClient(provider: KmsProvider | undefined, keyId: string | undefined): KmsClient {
	...
	// Placeholder clients for cloud KMS providers. Intentionally throw until wired.
	return {
		async encryptDek() {
			throw new Error('Cloud KMS encryptDek not implemented. Provide base64 32-byte KMS_KEY_ID for local wrapper.');
		},
		async decryptDek() {
			throw new Error('Cloud KMS decryptDek not implemented. Provide base64 32-byte KMS_KEY_ID for local wrapper.');
		},
	};
}
