...
export default function SignInPage() {
  ...
  const [correlationId] = useState(() => authAnalytics.startAuthSession());

  useEffect(() => {
    // Track page view
    authAnalytics.trackPageView('signin', correlationId, {
      hasError: !!error,
      errorType: error,
      isReauth: reauth,
      isExpired: expired
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // Track provider loading performance
          authAnalytics.trackPerformance('page_load', Date.now() - startTime, correlationId, {
            providersFound: Object.keys(json).length
          });
        }
      } catch (error) {
        if (!cancelled) {
          setProviders({});
          trackAuthError('Failed to load providers', 'network');
        }
      } finally {
  ...
  }, [correlationId, error, reauth, expired]);

  // Focus management for accessibility
  useEffect(() => {
    if (!loading && providers && !isSigningIn && firstButtonRef.current) {
      firstButtonRef.current.focus();
    }
  }, [loading, providers, isSigningIn]);
  ...
  // Get appropriate message based on URL params
  ...
  return (
    <main className="min-h-screen relative overflow-hidden" role="main">
      {/* Background with calm gradient and subtle wave pattern */}
